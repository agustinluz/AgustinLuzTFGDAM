...
const fetchDatos = async () => {
  try {
    ...
    // Cargar gastos
    const resGastos = await fetch(`${import.meta.env.VITE_API_URL}/grupos/${grupoId}/gastos`)
    if (resGastos.ok) {
...
<style scoped>
.deudas-card, .gastos-card {
  margin-bottom: 16px;
}

.deudas-header, .gastos-header {
  display: flex;
  align-items: center;
...
.participante-monto.deuda {
  color: #e74c3c;
}

.participante-monto.credito {
  color: #27ae60;
}
